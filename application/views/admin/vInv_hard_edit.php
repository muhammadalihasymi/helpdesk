...
<body id="page-top">
    ...
    <div class="wrapper2">
        ...
        <h2 style=" text-align: center ; margin-bottom: 50px; font-weight: bold; color: rgb(0, 128, 192)">Edit Inventory Hardware</h2>

        <!-- CONTENT HERE -->
        <div class="container">
            <?php foreach ($hardware as $m) : ?>
                <form action="<?php echo base_url() . 'cInv_hard/update'; ?>" method="post">
                    <table style="margin:20px auto;">
                        <tr>
                            <td><input class="form-control" type="hidden" name="id" value="<?php echo $m->id_hard ?>" readonly></td>
                        </tr>
                        <tr>
                            <td>Item</td>
                            <td style="padding-bottom: 10px">
                                <select class="form-control" name="item">
                                    <?php
                                    foreach ($hard->result() as $j) {
                                        $selected = '';
                                        if ($j->id == $m->item_id) {
                                            $selected = 'selected';
                                        }
                                        echo "<option value='" . $j->id . "' " . $selected . ">" . $j->item . "</option>";
                                    }
                                    ?>
                                </select>
                            </td>
                        ...
                        <tr>
                            <td>Type</td>
                            <td style="padding-bottom: 10px">
                                <input class="form-control" type="text" name="type" value="<?php echo $m->type ?>">
                            </td>
                        </tr>
                        <tr>
                            <td>Total</td>
                            <td style="padding-bottom: 10px">
                                <input class="form-control" type="number" name="total" value="<?php echo $m->total ?>">
                            </td>
                        </tr>
                        <tr>
                            <td>Condition</td>
                            <td style="padding-bottom: 10px">
                                <select class="form-control" name="con">
                                    <option <?= ($m->con_id == 'Used') ? 'selected' : ''; ?>>Used</option>
                                    <option <?= ($m->con_id == 'Ready') ? 'selected' : ''; ?>>Ready</option>
                                    <option <?= ($m->con_id == 'Repair') ? 'selected' : ''; ?>>Repair</option>
                                    <option <?= ($m->con_id == 'Damaged') ? 'selected' : ''; ?>>Damaged</option>
                                    <option <?= ($m->con_id == 'Cannot') ? 'selected' : ''; ?>>Cannot</option>
                                </select>
                            </td>
                        </tr>
                        <tr>
        ...
        <a href="<?= base_url('cInv_hard'); ?>">
            <a style="
                    position: absolute;
                    bottom: 20px;
                    right: 50px" href="<?= base_url('cInv_hard'); ?>">Back to Inventory Hardware</a>
            <span style="
                position: absolute;
                bottom: 20px;
